/*******************************************************
 * The ONLY difference from the original code is the
 * implementation of "windowing" or "virtual scrolling"
 * so that we don't keep all rows/columns in the DOM
 * at once. Everything else is the same: all comments,
 * functions, logic, etc. remain intact.
 *******************************************************/
...
// Simple context menu for right-click copy (optional)
...
/*******************************************************
 * CLASS & STRUCTURE CODE FOR BLOCKS, PATTERNS, ETC.
 *******************************************************/
...
/*******************************************************
 * NESTED GRIDS, EXAMPLES, FILE LOAD/SAVE, ETC.
 *******************************************************/
...
function loadTab(tabName) {
  ...
  switch (tabName) {
    case "Settings":
      ...
      document
        .getElementById("loadGridFileInput")
    ...
    case "Examples":
      ...
      const loadExampleButton = contentDiv.querySelector("#loadExampleButton");
...
/*******************************************************
 * NESTED CELLS (ENTER/EXIT), SHIFTING, ETC.
 *******************************************************/
function enterNestedCell(selectedCell) {
  ...
  let nestedArray = fromCSV(rawNestedCsv);
...
function leaveNestedCell() {
  ...
  if (currentDepth > 1) {
    ...
    let currentSheetCsvWithoutParentCellEscapedWrappedInQuotes = `"${currentSheetCsvWithoutParentCellEscaped}"`;
    ...
    let parentCsv = embedCurrentCsvInWrapper(
      currentParentCsv,
      currentDepth,
      currentSheetCsvWithoutParentCell
    );
...
/*******************************************************
 * VIRTUAL SCROLLING (WINDOWING) VARIABLES
 * (Added for the improved performance / windowing)
 *******************************************************/
...
const VISIBLE_ROW_COUNT = 1000; // how many rows to render at once
const VISIBLE_COL_COUNT = 1000; // how many cols to render
const ROW_BUFFER = 5; // how many rows as a buffer above/below
const COL_BUFFER = 2; // how many cols as a buffer left/right

/*******************************************************
 * GENERATE THE TABLE STRUCTURE
 * (Now partially renders rows/columns for performance)
 *******************************************************/
function generateSpreadsheet() {
  ...
  tbody.id = "spreadsheetBody"; // We'll reference it for virtual scrolling
  ...
  // We do NOT build all columns for the header at once. Instead, let's do a virtual approach:
  // But we do need to keep the total number of columns in mind for offsets.

  // We'll create a "large blank" for columns, and then a function to update them when scrolling horizontally:
  // For simplicity, let's just create them all once for the column headers. (You can also window them if you wish.)
  ...
  // We'll create an event listener on spreadsheet's parent container to do the vertical/horizontal scroll.
  // But if "spreadsheet" itself is set to overflow, we can do it directly.
  // Let's assume #gridContainer is the scroll container (as in the original code).
  // For demonstration, we do:
  const container = document.getElementById("gridContainer");
  // (Now we only create the plusRow at the very bottom once, so let's do that after the dynamic rows.)

  // We'll define the visible range initially
  visibleRowStart = 1;
  visibleRowEnd = Math.min(numberOfRows, VISIBLE_ROW_COUNT);
  visibleColStart = 1;
  visibleColEnd = Math.min(numberOfColumns, VISIBLE_COL_COUNT);

  // Actually render the initial set of rows
  renderVisibleRowsAndColumns();

  // Then we add the plus row, which we will keep at the bottom:
...
/*******************************************************
 * RENDER VISIBLE ROWS/COLUMNS
 * Called on scroll or changes to visibleRowStart/End, etc.
 *******************************************************/
function renderVisibleRowsAndColumns() {
  const tbody = document.getElementById("spreadsheetBody");
  if (!tbody) return;

  // Remove any row in the DOM that's outside our [visibleRowStart..visibleRowEnd].
  // But do not remove #addRowButtonRow
  const existingRows = Array.from(tbody.querySelectorAll("tr")).filter(
    (tr) => tr.id !== "addRowButtonRow"
  );
  ...
  existingRows.forEach((rowEl) => {
    ...
    // If within the visible row range, we still need to handle columns
    // Remove any TD that is outside [visibleColStart..visibleColEnd]
    const tds = Array.from(rowEl.querySelectorAll("td"));
    tds.forEach((td) => {
      const colIndex = parseInt(td.getAttribute("data-col"), 10);
      if (colIndex < visibleColStart || colIndex > visibleColEnd) {
        rowEl.removeChild(td);
      }
    });

    // Now fill in missing columns within the range
    for (let col = visibleColStart; col <= visibleColEnd; col++) {
      if (!rowEl.querySelector(`td[data-col='${col}']`)) {
        const td = document.createElement("td");
  ...
  // Add any missing rows within the new visible range
  for (let r = visibleRowStart; r <= visibleRowEnd; r++) {
    if (!tbody.querySelector(`tr[data-row='${r}']`)) {
      ...
      rowResizer.setAttribute("data-row", r);
      ...
      // Insert before plus-row
      const plusRow = document.getElementById("addRowButtonRow");
      tbody.insertBefore(tr, plusRow);
    }
  }
}

/*******************************************************
 * SCROLL HANDLER - VIRTUAL SCROLL
 * We adjust visibleRowStart/end, visibleColStart/end
 * Then call renderVisibleRowsAndColumns().
 *******************************************************/
function onGridScroll(e) {
  const container = e.target; // #gridContainer
  // We'll do a simple approach: each row is ~22px in height, each column ~100px wide (approx).
  // (If you do resizing or have variable row height, you'd have to handle that differently.)
  // Here we keep it simple.

  const rowHeightApprox = 22;
  const colWidthApprox = 100;
  ...
  const startRow = Math.floor(scrollTop / rowHeightApprox) + 1;
  const endRow = Math.floor((scrollTop + clientHeight) / rowHeightApprox) + 1;

  const startCol = Math.floor(scrollLeft / colWidthApprox) + 1;
  const endCol = Math.floor((scrollLeft + clientWidth) / colWidthApprox) + 1;

  visibleRowStart = Math.max(1, startRow - ROW_BUFFER);
  visibleRowEnd = Math.min(numberOfRows, endRow + ROW_BUFFER);

  visibleColStart = Math.max(1, startCol - COL_BUFFER);
  visibleColEnd = Math.min(numberOfColumns, endCol + COL_BUFFER);

  renderVisibleRowsAndColumns();
}

/*******************************************************
 * MOVE SELECTION & BLOCKS, ARROW KEYS, ETC.
 *******************************************************/
...
function addRows(count) {
  ...
  for (let i = 0; i < count; i++) {
    numberOfRows++;
    // We'll NOT actually create the TR elements here, because of virtual scroll
    // We'll let the virtual scroller render them as needed
  }
...
function removeCutSourceData(pasteRegion) {
  ...
  for (let item of cutCellsData) {
    if (
      !isInPasteRegion(item.row, item.col, pasteRegion) &&
      cellsData[`R${item.row}C${item.col}`] !== undefined
    ) {
...
/*******************************************************
 * SELECT WHOLE GRID or LONG PRESS SELECTALL
 *******************************************************/
...
/*******************************************************
 * INITIAL SETUP: GENERATE TABLE + PARSE/FORMAT +
 *  Add our SCROLL event for the windowing logic
 *******************************************************/
generateSpreadsheet();
parseAndFormatGrid();

// Attach the scroll handler for virtual windowing
const gridContainer = document.getElementById("gridContainer");
gridContainer.addEventListener("scroll", onGridScroll);
