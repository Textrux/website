...
function testTreeWithTableDomain() {
  ...
  if (tree?.parentElements.length > 0) {
    ...
    console.log(`   First parent: "${firstParent.content}" at (${firstParent.position.row}, ${firstParent.position.col}) level ${firstParent.level}`);
    console.log(`   First parent children: ${firstParent.children.length}`);
    firstParent.children.forEach((child, i) => {
      console.log(`     Child ${i}: "${child.content}" at (${child.position.row}, ${child.position.col}) level ${child.level}`);
    });
    
    if (firstParent.domainRegion) {
      console.log(`   First parent domain: rows ${firstParent.domainRegion.topRow}-${firstParent.domainRegion.bottomRow}, cols ${firstParent.domainRegion.leftCol}-${firstParent.domainRegion.rightCol}`);
      console.log(`   Nested construct type: ${firstParent.domainRegion.nestedConstruct || 'none'}`);
      console.log(`   Parse success: ${firstParent.domainRegion.parsedSuccessfully}`);
    }
  }
